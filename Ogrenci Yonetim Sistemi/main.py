...
def ogrenciBulSil():
    okulAd = input("Silmek istediğiniz öğrencinin okulu: ")
    okulNo = input("Silmek istediğiniz öğrencinin okul numarası: ")
    try:
        with open("db/"+okulAd+".txt","r") as okul:
            veri = okul.read()
        if("$"+okulNo in veri):
            print("Öğrenci bulundu")
            ogrenciSil(okulAd,okulNo)
        else:
            print(f"{okulNo} numaralı bir öğrenci kayıtlı değil")
    except FileNotFoundError:
        print(f"{okulAd} okulu kayıtlı değil")

def ogrenciSil(okulAd,okulNo):
    with open("db/"+okulAd+".txt","r",encoding="utf-8") as okul:
        satirlar = okul.readlines()
        for i in satirlar:
            if "$"+okulNo in i:
